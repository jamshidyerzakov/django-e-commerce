...
from .models import (
    ...
    DriverMore,
    AdminMore,
    ModeratorMore, Seller, Customer, Driver, Moderator, Admin
)
...
class CustomerAdminCreationForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        customer.save()
        CustomerMore.objects.create(user=customer)
        return customer


class DriverAdminCreationForm(forms.ModelForm):
    password1 = forms.CharField(label='Password', widget=forms.PasswordInput)
    password2 = forms.CharField(label='Password confirmation', widget=forms.PasswordInput)
    ...
    def save(self, commit=True):
        driver = super(DriverAdminCreationForm, self).save(commit=False)
        driver.set_password(self.cleaned_data["password1"])
        driver.save()
        DriverMore.objects.create(user=driver)
        return driver
...
class DriverAdminChangeForm(forms.ModelForm):
...
class AdminChangeForm(forms.ModelForm):
    """A form for updating admins. Includes all the fields on
    the user, but replaces the password field with admin's
    password hash display field.
    """
    password = ReadOnlyPasswordHashField()
